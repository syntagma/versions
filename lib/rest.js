...
var routes = {
  ...
  '/version': function version(req, res, next) {
    ...
    var allowedVersionsLastValue = this.config['allowed versions'];
    this.set('allowed versions',new Array(this.get('version')));
    this.emit('change:allowed versions', allowedVersionsLastValue, this.config['allowed versions']);

    res.JSON({ version: body.version });
  },

    /**
     * CRUD for allowed versions.
     */
    '/allowed-versions': function version(req, res, next) {
        var body = req.body;
        if (req.method === 'GET') {
            return res.JSON({ 'allowed versions': this.get('allowed versions') });
        } else if (req.method === 'DELETE') {
            var allowedVersionsLastValue = this.config['allowed versions'];
            this.config['allowed versions']= new Array(this.get('version'));
            this.emit('change:allowed versions', allowedVersionsLastValue, this.config['allowed versions']);
            res.JSON({ 'allowed versions': this.get('allowed versions') });
        } else {
            if (!body || !('allowed versions' in body)) return res.JSON({ error: 'Invalid body' });

            if (req.method === 'POST') {
                this.config['allowed versions'] = new Array(this.get('version'));
            }
            this.set('allowed versions', body['allowed versions']);

            this.emit('sync:allowed versions', body['allowed versions'], this.get('allowed versions'));
            res.JSON({ 'allowed versions': this.get('allowed versions') });
        }
    },

    /**
     * CRUD for autoexpired resources.
     */
    '/autoexpired-resources': function version(req, res, next) {
        var body = req.body;
        if (req.method === 'GET') {
            return res.JSON({ 'autoexpired resources': this.get('autoexpired resources') });
        } else if (req.method === 'DELETE') {
            var autoexpiredResourcesLastValue = this.config['autoexpired resources'];
            this.emit('change:allowed versions', autoexpiredResourcesLastValue, []);
            res.JSON({ 'autoexpired resources': this.get('autoexpired resources') });
        } else {
            if (!body || !('autoexpired resources' in body)) return res.JSON({ error: 'Invalid body' });

            if (req.method === 'POST') {
                this.config['autoexpired resources'] = new Array();
            }
            this.set('autoexpired resources', body['autoexpired resources']);

            this.emit('sync:autoexpired resources', body['autoexpired resources'], this.get('autoexpired resources'));
            res.JSON({ 'allowed versions': this.get('autoexpired resources') });
        }
    },
